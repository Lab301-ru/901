/* ─────────────────────────────────────────────────────────────
   LAB301 — SEO module · TAB REGISTRY
  ...
  const siteDefaultsSections = [
    {
      ...
      fields: [
        t('titleTemplate', 'Шаблон заголовка', {
          hint: '⚠️ %pageTitle% и %siteName% — это МЕТКИ, их не нужно заменять своим текстом: ' +
                'вместо них подставится заголовок конкретной статьи и название сайта. ' +
                'Пишите текст ВОКРУГ меток. Пример: «%pageTitle% — Новости LAB301» даст ' +
                '«Аудит сайта — Новости LAB301». Если метку %pageTitle% убрать, шаблон ' +
                'будет проигнорирован — иначе у всех страниц оказался бы одинаковый заголовок.',
          example: '%pageTitle% — Новости LAB301',
          validate: v => (v && v.indexOf('%pageTitle%') === -1)
            ? 'Нет метки %pageTitle% — шаблон не применится. Добавьте её в строку.'
            : ''
        }),
