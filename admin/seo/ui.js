...
(function (root) {
  ...
  const LS_SECTIONS = 'lab301_seo_sections';
  ...
  function mount(container, opts) {
    ...
    let activeId = (localStorage.getItem('lab301_seo_tab') || 'basic');
      ...
      F.groups().forEach(g => {
        ...
        F.tabs.filter(t => t.group === g).forEach(tab => {
          const btn = el('button.seo-tab' + (tab.id === activeId ? '.active' : ''), {
            ...
            onClick: () => { activeId = tab.id; localStorage.setItem('lab301_seo_tab', tab.id); render(); }
