/* ─────────────────────────────────────────────────────────────
   LAB301 — SEO module · APPLICATION layer
...
(function (root, factory) {
  ...
  function resolveSeo(post, site) {
    ...
    const url = first(seo.canonicalUrl && abs(seo.canonicalUrl), origin + '/news/' + post.slug + '.html');
    ...
    return {
      ...
      schemaType: first(seo.schemaType, def.schemaType, 'Article'),
      ...
      publisherLogo: abs(first(seo.publisherLogo, def.publisherLogo, '/apple-touch-icon.png')),
  ...
  function defaultBreadcrumbs(post, origin) {
    return [
      ...
      { label: 'Новости', url: origin + '/news.html' },
      { label: plain(post.title), url: origin + '/news/' + post.slug + '.html' }
    ];
  }
   ...
   */
  /**
   * robots.txt в стиле LAB301: AI-краулеры перечисляются ЯВНО.
   * Явный `Allow` для разрешённых ботов — сигнал answer-движкам, что
   * контент можно использовать в ответах; молчаливое отсутствие правила
   * часть краулеров трактует консервативнее. Запрещённые получают
   * `Disallow: /`. Админка закрыта всегда.
   */
  function robotsTxt(site, aiDefaults) {
    const ai = aiDefaults || {};
    let out = '# Default: allow everything except the admin panel\n' +
              'User-agent: *\nAllow: /\nDisallow: /admin/\n';
    ...
    const allowed = (ai.allowedAiCrawlers || []).filter(b => !disallowed.has(b));
    allowed.forEach(b => disallowed.delete(b));

    if (allowed.length) {
      out += '\n# --- AI / LLM crawlers: explicitly allowed (answer engines & AI search) ---\n';
      allowed.forEach(bot => {
        out += 'User-agent: ' + bot + '\nAllow: /\nDisallow: /admin/\n\n';
      });
    }
  ...
  function autoGenerateCanonical(post, site) {
    return (site.origin || '') + '/news/' + post.slug + '.html';
  }
